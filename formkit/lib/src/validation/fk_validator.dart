...
///
/// ```dart
/// final validator = Fk.required().and(Fk.minLength(8));
...
abstract class FkValidator<T> {
  ...
  /// ```
  FkValidator<T> and(FkValidator<T> other) => _AndValidator<T>(this, other);
  ...
  /// ```
  FkValidator<T> or(FkValidator<T> other) => _OrValidator<T>(this, other);
  ...
  ///
  /// The [predicate] receives the [FkValidationContext] (if any) so it can
  ...
  /// });
  /// ```
  FkValidator<T> when(bool Function(FkValidationContext? context) predicate) =>
...
class _AndValidator<T> extends FkValidator<T> {
  final FkValidator<T> _first;
  final FkValidator<T> _second;
...
class _OrValidator<T> extends FkValidator<T> {
  final FkValidator<T> _first;
  final FkValidator<T> _second;
...
class _ConditionalValidator<T> extends FkValidator<T> {
  final FkValidator<T> _validator;
  final bool Function(FkValidationContext? context) _predicate;
