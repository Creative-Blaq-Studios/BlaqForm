...
import 'fk_field_controller.dart';

/// Aggregates [FkFieldController]s, handles cross-field validation, and gates
/// form submission.
///
/// ```dart
...
/// });
/// ```
class FkFormController extends ChangeNotifier {
  ...
  /// password").
  FkFormController({
    List<FkValidator<Map<String, dynamic>>> crossValidators = const [],
  ...
  final Map<String, FkFieldController> _fields = {};
  bool _isSubmitting = false;
  final List<FkValidationResult> _crossErrors = [];
  ...
  void register(String name, FkFieldController controller) {
  ...
  FkFieldController<T> field<T>(String name) {
    final controller = _fields[name];
    if (controller == null) {
      throw StateError(
        'FkFormController: no field registered with name "$name".',
      );
    }
    return controller as FkFieldController<T>;
  }
  ...
  Map<String, FkValidationResult?> get errors {
    return {
  ...
  List<FkValidator<Map<String, dynamic>>> crossValidators;
  ...
  Future<bool> submit(
    ...
    log.info(FkLogCategory.submission, 'Submit started');
    ...
    try {
      ...
      log.info(FkLogCategory.submission, 'Submit completed successfully');
      return true;
    } catch (e) {
      log.error(
        FkLogCategory.submission,
  ...
  void reset() {
    ...
    FkLogger.instance.info(FkLogCategory.reset, 'Form reset');
    notifyListeners();
  }
