...
/// A submit button that integrates with [FkFormController].
///
/// Automatically disables when the form is invalid or submitting, and shows a
/// loading indicator during submission.
///
/// Must be placed inside an [FkForm] so it can look up the controller via
/// [FkForm.of].
///
/// ```dart
/// FkSubmitButton(
...
/// )
/// ```
class FkSubmitButton extends StatelessWidget {
  /// Creates a submit button that integrates with the nearest [FkForm].
  ///
  /// [onSubmit] is called when the button is pressed and receives the form's
  /// [FkFormController].
  const FkSubmitButton({
    super.key,
  ...
  ///
  /// Typically calls [FkFormController.submit] internally:
  /// ```dart
  /// onSubmit: (controller) => controller.submit((values) async { ... }),
  /// ```
  final Future<void> Function(FkFormController controller) onSubmit;
  ...
  Widget build(BuildContext context) {
    final controller = FkForm.of(context).controller;
